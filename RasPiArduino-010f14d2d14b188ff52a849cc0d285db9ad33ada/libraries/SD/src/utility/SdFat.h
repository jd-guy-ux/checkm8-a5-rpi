...
uint8_t const F_READ = 0X01;
/** open() oflag - same as F_READ */
uint8_t const F_RDONLY = F_READ;
/** open() oflag for write */
uint8_t const F_WRITE = 0X02;
/** open() oflag - same as F_WRITE */
uint8_t const F_WRONLY = F_WRITE;
/** open() oflag for reading and writing */
uint8_t const F_RDWR = (F_READ | F_WRITE);
/** open() oflag mask for access modes */
uint8_t const F_ACCMODE = (F_READ | F_WRITE);
/** The file offset shall be set to the end of the file prior to each write. */
uint8_t const F_APPEND = 0X04;
/** synchronous writes - call sync() after each write */
uint8_t const F_SYNC = 0X08;
/** create the file if nonexistent */
uint8_t const F_CREAT = 0X10;
/** If F_CREAT and F_EXCL are set, open() shall fail if the file exists */
uint8_t const F_EXCL = 0X20;
